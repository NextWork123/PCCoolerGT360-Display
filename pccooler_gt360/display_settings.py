...
def set_brightness(device: "SerialDevice", seq: int, level: int, verbose: bool = False) -> bool:
    """Set display brightness level (0-100) (synchronous).
...
async def set_brightness_async(device: "AsyncSerialDevice", seq: int, level: int, verbose: bool = False) -> bool:
    """Set display brightness level (0-100) (asynchronous).

    Args:
        device: Async serial device wrapper
        seq: Sequence number for packet
        level: Brightness level 0-100 (0=off, 100=max)
        verbose: Enable verbose logging
    ...
    """
    level = max(0, min(100, level))  # Clamp to valid range
    log_message(f"Setting brightness to {level}% (async)...", "INFO", verbose)
...
def set_orientation(device: "SerialDevice", seq: int, angle: int, verbose: bool = False) -> bool:
    """Set display orientation/rotation angle (synchronous).
...
async def set_orientation_async(device: "AsyncSerialDevice", seq: int, angle: int, verbose: bool = False) -> bool:
    """Set display orientation/rotation angle (asynchronous).
    ...
    log_message(f"Setting orientation to {angle}° (async)...", "INFO", verbose)
...
def probe_orientation_protocol(device: "SerialDevice", seq: int, verbose: bool = False) -> dict:
    """Probe the device to find the correct orientation protocol (synchronous).
...
async def probe_orientation_protocol_async(device: "AsyncSerialDevice", seq: int, verbose: bool = False) -> dict:
    """Probe the device to find the correct orientation protocol (asynchronous).
    ...
    results = {}
    for idx, (endpoint, body) in enumerate(variants):
        log_message(f"Probing: {endpoint} {body} (async)...", "INFO", verbose)
        ack = await send_command_async(
            device,
            endpoint,
            body,
